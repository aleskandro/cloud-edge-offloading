...
def usage_vectors(simulate, naive=False):
    Random.seed(2)
    cmpilph.generate_input_datas(K=2)
    ...
    if naive:
        ...
        df.to_csv("results/usage_vectors.csv")
    ...
    def quiv(df, color="r"):
        ...
        for index, row in df.iterrows():
            ...
            ax.quiver([last_coordinate[int(row["Node"])][0]],
                      ...
                      [row["CPU"]],
                      [row["RAM"]], angles='xy', scale_units='xy', scale=1, width=0.0025, color=[color])
